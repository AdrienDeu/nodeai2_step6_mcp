 ...
 */
function makeAbortOnDisconnect(request) {
  const controller = new AbortController()
  const onClose = () => controller.abort()
  request.socket.once('close', onClose)
  // Retourne aussi un cleanup pour retirer le listener après usage
  const cleanup = () => request.socket.removeListener('close', onClose)
  return { controller, cleanup }
}

/** @param {import('fastify').FastifyInstance} app */
export async function agentRoute(app) {
  /**
   * POST /chat/agent
      ...
      for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        ...
        for (const tc of toolCalls) {
          ...
          try {
            ...
            request.log.warn({ name, err: err.message }, 'Tool error')
          }
